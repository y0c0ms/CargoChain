...
const PACKAGE_ABI = [
  ...
  "function hopCount() view returns (uint256)",
];
...
export default function Carrier() {
  ...
  async function resolvePackage(signer: ethers.Signer) {
  ...
  async function transfer() {
    try {
      ...
      const pkg = await resolvePackage(signer);
      const handshake = ethers.keccak256(ethers.toUtf8Bytes(`${Date.now()}`));

      setStatus("transferring custody...");
      const tx = await pkg.transferCustody(toAddr, loc, handshake);
      await tx.wait();
      const hops = await pkg.hopCount();
  ...
  async function markDelivered() {
    try {
      ...
      const tx = await pkg.markDelivered();
      await tx.wait();
    ...
    <main className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-slate-600 mb-6">
          Transfer custody to the next carrier, or mark the package delivered
          when it reaches its destination. Both actions must be signed by the{" "}
          <strong>current holder</strong> — use the picker in the top-right to
          switch identities.
        </p>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 space-y-4">
          ...
          <fieldset className="border border-slate-200 rounded-lg p-4">
            ...
            <p className="text-xs text-slate-500 mb-2">
              Only the current holder can mark a package delivered.
            </p>
            <button
